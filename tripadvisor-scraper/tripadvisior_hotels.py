...
Install PhantomJS or get Chrome- or Firefox webdriver binaries and add them to your PATH (see http://selenium-python.readthedocs.io/installation.html#drivers)
        ...
        python tripadvisior_hotels.py -e firefox -n 10 -o my_hotels_reviews.csv https://www.tripadvisor.com/Hotels-g294452-Sofia_Sofia_Region-Hotels.html
'''
import argparse
import datetime
import locale
...
URL_PATTERN = 'http(s)?:\/\/.?(www\.)?tripadvisor\.(com|de)\/Hotel.*'
...
class TripadvisorScraper():
    ...
    def _parse_page(self):
        ...
        for script in scripts:
            ...
            try:
                lat = re.search('"latitude":"(\\d*\\.?\\d*)"', html, re.IGNORECASE)
                lng = re.search('"longitude":"(\\d*\\.?\\d*)"', html, re.IGNORECASE)
            except: pass
        ...
        for e in review_elements:
            try:
                ...
                score = self.driver.find_element_by_class_name('is-9').find_element_by_class_name('ui_bubble_rating')
...
if __name__ == '__main__':
    ...
    urls = scraper.get_urls(args.url, 5)
    dfs = []
    for url in urls:
